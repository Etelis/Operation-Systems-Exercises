...
void test_preappend() {
    const char *test_name = "TEST_PREAPPEND";
    buffered_file_t *bf = buffered_open("buffered_testfile.txt", O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (bf == NULL) {
        write_result(test_name, "FAILED");
        return;
    ...
    const char *append_data = " with appended data";
    bytes_written = buffered_write(bf, append_data, strlen(append_data));
    if (bytes_written != strlen(append_data)) {
        write_result(test_name, "FAILED");
        buffered_close(bf);
    ...
    char read_buffer[BUFFER_SIZE_1];
    int bytes_read = buffered_read(bf, read_buffer, BUFFER_SIZE_1 - 1);
    if (bytes_read <= 0 || strcmp(read_buffer, "Initial data with appended data") != 0) {
        write_result(test_name, "FAILED");
        buffered_close(bf);
        return;
    }
...
void test_create_file() {
    const char *test_name = "TEST_CREATE_FILE";
    ...
    if (bytes_read != 13 || strncmp(buffer, "Hello, World!", 13) != 0) {
        write_result(test_name, "FAILED");
        close(fd);
        return;
    }
...
void test_read_only() {
    const char *test_name = "TEST_READ_ONLY";
    ...
    char buffer[14];
    ssize_t bytes_read = read(fd, buffer, 13);
    if (bytes_read != 13 || strcmp(buffer, "Hello, World!") != 0) {
        write_result(test_name, "FAILED");
        close(fd);
        return;
    }
...
void test_write_only() {
    const char *test_name = "TEST_WRITE_ONLY";
    ...
    ssize_t bytes_written = write(fd, "Goodbye, World!", 15);
    if (bytes_written != 15) {
        write_result(test_name, "FAILED");
        close(fd);
        return;
    ...
    char buffer[16];
    ssize_t bytes_read = read(fd, buffer, 15);
    if (bytes_read != 15 || strcmp(buffer, "Goodbye, World!") != 0) {
        write_result(test_name, "FAILED");
        close(fd);
        return;
    }
...
void test_append() {
    const char *test_name = "TEST_APPEND";
    ...
    ssize_t bytes_written = write(fd, " Append!", 8);
    if (bytes_written != 8) {
        write_result(test_name, "FAILED");
        close(fd);
        return;
    ...
    if (bytes_read != 23 || strcmp(buffer, "Goodbye, World! Append!") != 0) {
        write_result(test_name, "FAILED");
        close(fd);
        return;
    }
